...
#define valgrind_path   "valgrind --expensive-definedness-checks=yes \
                        --exit-on-first-error=yes  --error-exitcode=100 "
...
void init_cmdline(afl_state_t *afl){
    ...
    char *accurate_binary = getenv("AFL_CQMSAN_ACCURATE_TARGET");
    if (!accurate_binary) {
      FATAL("Environment variable AFL_CQMSAN_ACCURATE_TARGET not set. "
            "Please specify the path to the binary for accurate detection/valgrind.");
    }
    
    if (valgrind_mode) {
      sprintf(cmdline + strlen(cmdline), "%s ", accurate_binary);
    }
...
int qmsan_check_bugs(afl_state_t *afl){
  ...
  if(afl->fsrv.msan_bits && afl->fsrv.msan_bits[MAP_SIZE - 1]){
    if(check_msan_trace(afl, afl->fsrv.msan_bits)){
      ...
      if((!valgrind_mode && status == 1) || (valgrind_mode && status == 100)){
        afl->qmsan_crashes++;

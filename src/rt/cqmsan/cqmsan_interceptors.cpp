...
static THREADLOCAL int in_interceptor_scope = 0;

void __cqmsan_scoped_disable_interceptor_checks() { ++in_interceptor_scope; }
void __cqmsan_scoped_enable_interceptor_checks() { --in_interceptor_scope; }
...
struct DlsymAlloc : public DlSymAllocator<DlsymAlloc> {
  static bool UseImpl() { return !cqmsan_inited; }
};

#define ENSURE_CQMSAN_INITED() do { \
...
#define CHECK_UNPOISONED_0(x, n)                                  \
  do {                                                            \
    ...
    if (__offset >= 0 && __cqmsan::flags()->report_umrs) {         \
      ...
      __cqmsan::PrintWarningWithOrigin(                             \
          pc, bp, __cqmsan_get_origin((const char *)x + __offset)); \
...
#define CHECK_UNPOISONED_STRING(x, n)                           \
    CHECK_UNPOISONED_STRING_OF_LEN((x), __sanitizer::internal_strlen(x), (n))
...
INTERCEPTOR(SIZE_T, fread_unlocked, void *ptr, SIZE_T size, SIZE_T nmemb,
  ...
  if (res > 0)
    __cqmsan_unpoison(ptr, res *size);
  return res;
}
#define CQMSAN_MAYBE_INTERCEPT_FREAD_UNLOCKED INTERCEPT_FUNCTION(fread_unlocked)
#else
#define CQMSAN_MAYBE_INTERCEPT_FREAD_UNLOCKED
#endif

#if !SANITIZER_NETBSD
INTERCEPTOR(void *, mempcpy, void *dest, const void *src, SIZE_T n) {
  return (char *)__cqmsan_memcpy(dest, src, n) + n;
}
#define CQMSAN_MAYBE_INTERCEPT_MEMPCPY INTERCEPT_FUNCTION(mempcpy)
#else
#define CQMSAN_MAYBE_INTERCEPT_MEMPCPY
#endif
...
INTERCEPTOR(void *, bcopy, const void *src, void *dest, SIZE_T n) {
  return __cqmsan_memmove(dest, src, n);
}

INTERCEPTOR(int, posix_memalign, void **memptr, SIZE_T alignment, SIZE_T size) {
  GET_MALLOC_STACK_TRACE;
  CHECK_NE(memptr, 0);
  int res = cqmsan_posix_memalign(memptr, alignment, size, &stack);
  if (!res)
    __cqmsan_unpoison(memptr, sizeof(*memptr));
  return res;
}
...
INTERCEPTOR(void *, memalign, SIZE_T alignment, SIZE_T size) {
  GET_MALLOC_STACK_TRACE;
  return cqmsan_memalign(alignment, size, &stack);
}
#define CQMSAN_MAYBE_INTERCEPT_MEMALIGN INTERCEPT_FUNCTION(memalign)
#else
#define CQMSAN_MAYBE_INTERCEPT_MEMALIGN
#endif

INTERCEPTOR(void *, aligned_alloc, SIZE_T alignment, SIZE_T size) {
  GET_MALLOC_STACK_TRACE;
  return cqmsan_aligned_alloc(alignment, size, &stack);
}

#if !SANITIZER_NETBSD
INTERCEPTOR(void *, __libc_memalign, SIZE_T alignment, SIZE_T size) {
  GET_MALLOC_STACK_TRACE;
  void *ptr = cqmsan_memalign(alignment, size, &stack);
  if (ptr)
    DTLS_on_libc_memalign(ptr, size);
  return ptr;
}
#define CQMSAN_MAYBE_INTERCEPT___LIBC_MEMALIGN INTERCEPT_FUNCTION(__libc_memalign)
#else
#define CQMSAN_MAYBE_INTERCEPT___LIBC_MEMALIGN
#endif

INTERCEPTOR(void *, valloc, SIZE_T size) {
  GET_MALLOC_STACK_TRACE;
  return cqmsan_valloc(size, &stack);
}

#if !SANITIZER_FREEBSD && !SANITIZER_NETBSD
INTERCEPTOR(void *, pvalloc, SIZE_T size) {
  GET_MALLOC_STACK_TRACE;
  return cqmsan_pvalloc(size, &stack);
}
#define CQMSAN_MAYBE_INTERCEPT_PVALLOC INTERCEPT_FUNCTION(pvalloc)
#else
#define CQMSAN_MAYBE_INTERCEPT_PVALLOC
#endif
...
#  define CQMSAN_MAYBE_INTERCEPT_CFREE INTERCEPT_FUNCTION(cfree)
#else
#define CQMSAN_MAYBE_INTERCEPT_CFREE
#endif

#if !SANITIZER_NETBSD
INTERCEPTOR(__sanitizer::uptr, malloc_usable_size, void *ptr) {
  return __sanitizer_get_allocated_size(ptr);
}
#define CQMSAN_MAYBE_INTERCEPT_MALLOC_USABLE_SIZE \
...
static NOINLINE void clear_mallinfo(T *sret) {
  ...
  __cqmsan_unpoison(sret, sizeof(*sret));
}
#endif
...
#  define CQMSAN_MAYBE_INTERCEPT_MALLINFO INTERCEPT_FUNCTION(mallinfo)
#else
#  define CQMSAN_MAYBE_INTERCEPT_MALLINFO
#endif
...
#  define CQMSAN_MAYBE_INTERCEPT_MALLINFO2 INTERCEPT_FUNCTION(mallinfo2)
#else
#  define CQMSAN_MAYBE_INTERCEPT_MALLINFO2
#endif
...
#define CQMSAN_MAYBE_INTERCEPT_MALLOPT INTERCEPT_FUNCTION(mallopt)
#else
#define CQMSAN_MAYBE_INTERCEPT_MALLOPT
#endif
...
#define CQMSAN_MAYBE_INTERCEPT_MALLOC_STATS INTERCEPT_FUNCTION(malloc_stats)
#else
#define CQMSAN_MAYBE_INTERCEPT_MALLOC_STATS
#endif
...
#  define CQMSAN_MAYBE_INTERCEPT_STPNCPY INTERCEPT_FUNCTION(stpncpy)
#else
#define CQMSAN_MAYBE_INTERCEPT_STPCPY
#  define CQMSAN_MAYBE_INTERCEPT_STPNCPY
#endif
...
#define CQMSAN_MAYBE_INTERCEPT___STRDUP INTERCEPT_FUNCTION(__strdup)
#else
#define CQMSAN_MAYBE_INTERCEPT___STRDUP
#endif
...
INTERCEPTOR(char *, gcvt, double number, SIZE_T ndigit, char *buf) {
  ...
  SIZE_T n = __sanitizer::internal_strlen(buf);
  __cqmsan_unpoison(buf, n + 1);
  return res;
}
#define CQMSAN_MAYBE_INTERCEPT_GCVT INTERCEPT_FUNCTION(gcvt)
#else
#define CQMSAN_MAYBE_INTERCEPT_GCVT
#endif

INTERCEPTOR(char *, strcat, char *dest, const char *src) {
  ENSURE_CQMSAN_INITED();
  //GET_STORE_STACK_TRACE;
  SIZE_T src_size = __sanitizer::internal_strlen(src);
  SIZE_T dest_size = __sanitizer::internal_strlen(dest);
  ...
  __cqmsan_unpoison(dest + dest_size, src_size + 1);
  return res;
}
...
INTERCEPTOR(char *, strncat, char *dest, const char *src, SIZE_T n) {
  ...
  //__cqmsan_unpoison(dest + dest_size + copy_size, 1); // \0
  __cqmsan_unpoison(dest + dest_size, copy_size + 1);
  return res;
}
...
#define INTERCEPTOR_STRTO_BODY(ret_type, func, ...) \
  ...
  __cqmsan_unpoison(endptr, sizeof(*endptr));         \
  return res;
...
INTERCEPTOR(int, vswprintf, void *str, __sanitizer::uptr size, void *format, va_list ap) {
  ENSURE_CQMSAN_INITED();
  int res = REAL(vswprintf)(str, size, format, ap);
  if (res >= 0) {
    __cqmsan_unpoison(str, 4 * (res + 1));
  }
  return res;
}

INTERCEPTOR(int, swprintf, void *str, __sanitizer::uptr size, void *format, ...) {
  ENSURE_CQMSAN_INITED();
  va_list ap;
  va_start(ap, format);
...
#define INTERCEPTOR_STRFTIME_BODY(char_type, ret_type, func, s, ...) \
  ...
  if (s) __cqmsan_unpoison(s, sizeof(char_type) * (res + 1));          \
  return res;
...
#define CQMSAN_MAYBE_INTERCEPT___STRFTIME_L INTERCEPT_FUNCTION(__strftime_l)
#else
#define CQMSAN_MAYBE_INTERCEPT___STRFTIME_L
#endif
...
#define CQMSAN_MAYBE_INTERCEPT___WCSFTIME_L INTERCEPT_FUNCTION(__wcsftime_l)
#else
#define CQMSAN_MAYBE_INTERCEPT___WCSFTIME_L
#endif

INTERCEPTOR(int, mbtowc, wchar_t *dest, const char *src, SIZE_T n) {
  ENSURE_CQMSAN_INITED();
  int res = REAL(mbtowc)(dest, src, n);
  if (res != -1 && dest) __cqmsan_unpoison(dest, sizeof(wchar_t));
  return res;
}
...
INTERCEPTOR(SIZE_T, mbrtowc, wchar_t *dest, const char *src, SIZE_T n,
  ...
  if (res != (SIZE_T)-1 && dest) __cqmsan_unpoison(dest, sizeof(wchar_t));
  return res;
}
...
INTERCEPTOR(wchar_t *, wmemcpy, wchar_t *dest, const wchar_t *src, SIZE_T n) {
  ...
  __cqmsan_unpoison(dest, n * sizeof(wchar_t));
  return res;
}
...
#define CQMSAN_MAYBE_INTERCEPT_WMEMPCPY INTERCEPT_FUNCTION(wmempcpy)
#else
#define CQMSAN_MAYBE_INTERCEPT_WMEMPCPY
#endif
...
INTERCEPTOR(wchar_t *, wmemset, wchar_t *s, wchar_t c, SIZE_T n) {
  ...
  __cqmsan_unpoison(s, n * sizeof(wchar_t));
  return res;
}
...
INTERCEPTOR(int, gettimeofday, void *tv, void *tz) {
  ...
  if (tv)
    __cqmsan_unpoison(tv, 16);
  if (tz)
    __cqmsan_unpoison(tz, 8);
  return res;
}
...
INTERCEPTOR(char *, fcvt, double x, int a, int *b, int *c) {
  ...
  __cqmsan_unpoison(b, sizeof(*b));
  __cqmsan_unpoison(c, sizeof(*c));
  if (res)
    __cqmsan_unpoison(res, __sanitizer::internal_strlen(res) + 1);
  return res;
}
#define CQMSAN_MAYBE_INTERCEPT_FCVT INTERCEPT_FUNCTION(fcvt)
#else
#define CQMSAN_MAYBE_INTERCEPT_FCVT
#endif
...
static void UnpoisonEnviron() {
  char **envp = environ;
  for (; *envp; ++envp) {
    __cqmsan_unpoison(envp, sizeof(*envp));
    __cqmsan_unpoison(*envp, __sanitizer::internal_strlen(*envp) + 1);
  }
...
INTERCEPTOR(int, fstat, int fd, void *buf) {
  ...
  if (!res)
    __cqmsan_unpoison(buf, __sanitizer::struct_stat_sz);
  return res;
}
#  define CQMSAN_MAYBE_INTERCEPT_FSTAT CQMsan_INTERCEPT_FUNC(fstat)
#else
#define CQMSAN_MAYBE_INTERCEPT_FSTAT
#endif
...
INTERCEPTOR(int, fstat64, int fd, void *buf) {
  ...
  if (!res)
    __cqmsan_unpoison(buf, __sanitizer::struct_stat64_sz);
  return res;
}
#  define CQMSAN_MAYBE_INTERCEPT_FSTAT64 CQMsan_INTERCEPT_FUNC(fstat64)
#else
#  define CQMSAN_MAYBE_INTERCEPT_FSTAT64
#endif
...
#  define CQMSAN_MAYBE_INTERCEPT___FXSTAT CQMsan_INTERCEPT_FUNC(__fxstat)
#else
#define CQMSAN_MAYBE_INTERCEPT___FXSTAT
#endif
...
#  define CQMSAN_MAYBE_INTERCEPT___FXSTAT64 CQMsan_INTERCEPT_FUNC(__fxstat64)
#else
#  define CQMSAN_MAYBE_INTERCEPT___FXSTAT64
#endif
...
INTERCEPTOR(int, fstatat, int fd, char *pathname, void *buf, int flags) {
  ...
  if (!res) __cqmsan_unpoison(buf, __sanitizer::struct_stat_sz);
  return res;
}
#  define CQMSAN_MAYBE_INTERCEPT_FSTATAT CQMsan_INTERCEPT_FUNC(fstatat)
#else
#  define CQMSAN_MAYBE_INTERCEPT_FSTATAT
#endif
...
#  define CQMSAN_MAYBE_INTERCEPT_FSTATAT64 CQMsan_INTERCEPT_FUNC(fstatat64)
#else
#  define CQMSAN_MAYBE_INTERCEPT_FSTATAT64
#endif
...
#  define CQMSAN_MAYBE_INTERCEPT___FXSTATAT CQMsan_INTERCEPT_FUNC(__fxstatat)
#else
#  define CQMSAN_MAYBE_INTERCEPT___FXSTATAT
#endif
...
INTERCEPTOR(int, __fxstatat64, int magic, int fd, char *pathname, void *buf,
  ...
  if (!res) __cqmsan_unpoison(buf, __sanitizer::struct_stat64_sz);
  return res;
}
#  define CQMSAN_MAYBE_INTERCEPT___FXSTATAT64 CQMsan_INTERCEPT_FUNC(__fxstatat64)
#else
#  define CQMSAN_MAYBE_INTERCEPT___FXSTATAT64
#endif
...
INTERCEPTOR(int, pipe, int pipefd[2]) {
  ...
  if (!res)
    __cqmsan_unpoison(pipefd, sizeof(int[2]));
  return res;
}
...
INTERCEPTOR(int, socketpair, int domain, int type, int protocol, int sv[2]) {
  ...
  if (!res)
    __cqmsan_unpoison(sv, sizeof(int[2]));
  return res;
}
...
INTERCEPTOR(char *, fgets_unlocked, char *s, int size, void *stream) {
  ...
  if (res)
    __cqmsan_unpoison(s, __sanitizer::internal_strlen(s) + 1);
  return res;
}
#define CQMSAN_MAYBE_INTERCEPT_FGETS_UNLOCKED INTERCEPT_FUNCTION(fgets_unlocked)
#else
#define CQMSAN_MAYBE_INTERCEPT_FGETS_UNLOCKED
#endif
...
#define INTERCEPTOR_GETRLIMIT_BODY(func, resource, rlim)  \
  ...
  if (!res)                                               \
    __cqmsan_unpoison(rlim, __sanitizer::struct_rlimit_sz); \
  return res
...
INTERCEPTOR(int, getrlimit64, int resource, void *rlim) {
  if (cqmsan_init_is_running) return REAL(getrlimit64)(resource, rlim);
  ENSURE_CQMSAN_INITED();
  int res = REAL(getrlimit64)(resource, rlim);
  if (!res) __cqmsan_unpoison(rlim, __sanitizer::struct_rlimit64_sz);
  return res;
}
...
INTERCEPTOR(int, prlimit, int pid, int resource, void *new_rlimit,
  ...
  if (!res) __cqmsan_unpoison(old_rlimit, __sanitizer::struct_rlimit_sz);
  return res;
}
...
INTERCEPTOR(int, prlimit64, int pid, int resource, void *new_rlimit,
  ...
  if (!res) __cqmsan_unpoison(old_rlimit, __sanitizer::struct_rlimit64_sz);
  return res;
}

#define CQMSAN_MAYBE_INTERCEPT___GETRLIMIT INTERCEPT_FUNCTION(__getrlimit)
#define CQMSAN_MAYBE_INTERCEPT_GETRLIMIT64 INTERCEPT_FUNCTION(getrlimit64)
#define CQMSAN_MAYBE_INTERCEPT_PRLIMIT INTERCEPT_FUNCTION(prlimit)
#define CQMSAN_MAYBE_INTERCEPT_PRLIMIT64 INTERCEPT_FUNCTION(prlimit64)
#else
#define CQMSAN_MAYBE_INTERCEPT___GETRLIMIT
#define CQMSAN_MAYBE_INTERCEPT_GETRLIMIT64
#define CQMSAN_MAYBE_INTERCEPT_PRLIMIT
#define CQMSAN_MAYBE_INTERCEPT_PRLIMIT64
#endif
...
INTERCEPTOR(int, gethostname, char *name, SIZE_T len) {
  ...
  if (!res || (res == -1 && errno == errno_ENAMETOOLONG)) {
    ...
    if (real_len < len)
      ++real_len;
    __cqmsan_unpoison(name, real_len);
  }
  return res;
}
...
INTERCEPTOR(int, epoll_wait, int epfd, void *events, int maxevents,
  ...
  if (res > 0) {
    __cqmsan_unpoison(events, __sanitizer::struct_epoll_event_sz * res);
  }
  return res;
}
#define CQMSAN_MAYBE_INTERCEPT_EPOLL_WAIT INTERCEPT_FUNCTION(epoll_wait)
#else
#define CQMSAN_MAYBE_INTERCEPT_EPOLL_WAIT
#endif
...
#define CQMSAN_MAYBE_INTERCEPT_EPOLL_PWAIT INTERCEPT_FUNCTION(epoll_pwait)
#else
#define CQMSAN_MAYBE_INTERCEPT_EPOLL_PWAIT
#endif
...
INTERCEPTOR(void *, calloc, SIZE_T nmemb, SIZE_T size) {
  ...
  return cqmsan_calloc(nmemb, size, &stack);
}
...
INTERCEPTOR(void *, realloc, void *ptr, SIZE_T size) {
  ...
  GET_MALLOC_STACK_TRACE;
  return cqmsan_realloc(ptr, size, &stack);
}

INTERCEPTOR(void *, reallocarray, void *ptr, SIZE_T nmemb, SIZE_T size) {
  GET_MALLOC_STACK_TRACE;
  return cqmsan_reallocarray(ptr, nmemb, size, &stack);
}
...
INTERCEPTOR(void *, malloc, SIZE_T size) {
  ...
  GET_MALLOC_STACK_TRACE;
  return cqmsan_malloc(size, &stack);
}
...
void __cqmsan_copy_shadow(void *dest, const void *src, __sanitizer::uptr n) {
  //GET_STORE_STACK_TRACE;
...
static void *mmap_interceptor(Mmap real_mmap, void *addr, SIZE_T length,
  ...
  if (res != (void *)-1) {
    ...
    if (MEM_IS_APP(res) && MEM_IS_APP(end_res)) {
      __cqmsan_unpoison(res, rounded_length);
    } else {
      // Application has attempted to map more memory than is supported by
      // CQMsan. Act as if we ran out of memory.
...
INTERCEPTOR(int, getrusage, int who, void *usage) {
  ...
  if (res == 0) {
    __cqmsan_unpoison(usage, __sanitizer::struct_rusage_sz);
  }
  return res;
}
...
static void *CQMsanThreadStartFunc(void *arg) {
  CQMsanThread *t = (CQMsanThread *)arg;
  SetCurrentThread(t);
  t->Init();
...
INTERCEPTOR(int, pthread_create, void *th, void *attr, void *(*callback)(void*),
            void * param) {
  ENSURE_CQMSAN_INITED(); // for GetTlsSize()
  ...
  CQMsanThread *t = CQMsanThread::Create(callback, param);
  ScopedBlockSignals block(&t->starting_sigset_);
  int res = REAL(pthread_create)(th, attr, CQMsanThreadStartFunc, t);

  if (attr == &myattr)
    pthread_attr_destroy(&myattr);
  if (!res) {
    __cqmsan_unpoison(th, __sanitizer::pthread_t_sz);
  }
  return res;
}
...
INTERCEPTOR(int, pthread_key_create, __sanitizer_pthread_key_t *key,
  ...
  if (cqmsan_init_is_running) return REAL(pthread_key_create)(key, dtor);
  ENSURE_CQMSAN_INITED();
  int res = REAL(pthread_key_create)(key, dtor);
  if (!res && key)
    __cqmsan_unpoison(key, sizeof(*key));
  return res;
}
...
INTERCEPTOR(void, tzset, int fake) {
  ...
  REAL(tzset)(fake);
  if (tzname[0])
    __cqmsan_unpoison(tzname[0], __sanitizer::internal_strlen(tzname[0]) + 1);
  if (tzname[1])
    __cqmsan_unpoison(tzname[1], __sanitizer::internal_strlen(tzname[1]) + 1);
  return;
}

struct CQMSanAtExitRecord {
  void (*func)(void *arg);
  void *arg;
};
...
struct InterceptorContext {
  Mutex atexit_mu;
  Vector<struct CQMSanAtExitRecord *> AtExitStack;
...
void CQMSanAtExitWrapper() {
  CQMSanAtExitRecord *r;
  {
    Lock l(&interceptor_ctx()->atexit_mu);

    __sanitizer::uptr element = interceptor_ctx()->AtExitStack.Size() - 1;
...
void CQMSanCxaAtExitWrapper(void *arg) {
  UnpoisonParam(1);
  CQMSanAtExitRecord *r = (CQMSanAtExitRecord *)arg;
...
INTERCEPTOR(int, __cxa_atexit, void (*func)(void *), void *arg,
            void *dso_handle) {
  if (cqmsan_init_is_running) return REAL(__cxa_atexit)(func, arg, dso_handle);
...
static int setup_at_exit_wrapper(void(*f)(), void *arg, void *dso) {
  ENSURE_CQMSAN_INITED();
  CQMSanAtExitRecord *r =
      (CQMSanAtExitRecord *)InternalAlloc(sizeof(CQMSanAtExitRecord));
  ...
  if (!dso) {
    ...
    res = REAL(__cxa_atexit)((void (*)(void *a))CQMSanAtExitWrapper, 0, 0);
    if (!res) {
      interceptor_ctx()->AtExitStack.PushBack(r);
    }
  } else {
    res = REAL(__cxa_atexit)(CQMSanCxaAtExitWrapper, r, dso);
  }
  return res;
}
...
INTERCEPTOR(int, openpty, int *aparent, int *aworker, char *name,
  ...
  if (!res) {
    __cqmsan_unpoison(aparent, sizeof(*aparent));
    __cqmsan_unpoison(aworker, sizeof(*aworker));
  }
  return res;
}
#define CQMSAN_MAYBE_INTERCEPT_OPENPTY INTERCEPT_FUNCTION(openpty)
#else
#define CQMSAN_MAYBE_INTERCEPT_OPENPTY
#endif
...
#define CQMSAN_MAYBE_INTERCEPT_FORKPTY INTERCEPT_FUNCTION(forkpty)
#else
#define CQMSAN_MAYBE_INTERCEPT_FORKPTY
#endif

struct CQMSanInterceptorContext {
...
#define CHECK_UNPOISONED_CTX(ctx, x, n)                         \
  do {                                                          \
    if (!((CQMSanInterceptorContext *)ctx)->in_interceptor_scope) \
      CHECK_UNPOISONED_0(x, n);                                 \
  } while (0)

#define CQMsan_INTERCEPT_FUNC(name)                                       \
  do {                                                                  \
    if (!INTERCEPT_FUNCTION(name))                                      \
      VReport(1, "CompilerQEMUMemorySanitizer: failed to intercept '%s'\n", #name); \
  } while (0)

#define CQMsan_INTERCEPT_FUNC_VER(name, ver)                                 \
  do {                                                                     \
    if (!INTERCEPT_FUNCTION_VER(name, ver))                                \
      VReport(1, "CompilerQEMUMemorySanitizer: failed to intercept '%s@@%s'\n", #name, \
              ver);                                                        \
  } while (0)
#define CQMsan_INTERCEPT_FUNC_VER_UNVERSIONED_FALLBACK(name, ver)             \
  do {                                                                      \
    if (!INTERCEPT_FUNCTION_VER(name, ver) && !INTERCEPT_FUNCTION(name))    \
      VReport(1, "CompilerQEMUMemorySanitizer: failed to intercept '%s@@%s' or '%s'\n", \
              #name, ver, #name);                                           \
  } while (0)

#define COMMON_INTERCEPT_FUNCTION(name) CQMsan_INTERCEPT_FUNC(name)
#define COMMON_INTERCEPT_FUNCTION_VER(name, ver) \
  CQMsan_INTERCEPT_FUNC_VER(name, ver)
#define COMMON_INTERCEPT_FUNCTION_VER_UNVERSIONED_FALLBACK(name, ver) \
  CQMsan_INTERCEPT_FUNC_VER_UNVERSIONED_FALLBACK(name, ver)
#define COMMON_INTERCEPTOR_UNPOISON_PARAM(count)  \
  UnpoisonParam(count)
#define COMMON_INTERCEPTOR_WRITE_RANGE(ctx, ptr, size) \
  __cqmsan_unpoison(ptr, size)
// To disable:  do { (void)(ctx); (void)(ptr); (void)(size); } while (false)
...
#define COMMON_INTERCEPTOR_INITIALIZE_RANGE(ptr, size) \
  __cqmsan_unpoison(ptr, size)
// To disable : do { (void)(ptr); (void)(size); } while (false)

#define COMMON_INTERCEPTOR_ENTER(ctx, func, ...)              \
  if (cqmsan_init_is_running)                                   \
    return REAL(func)(__VA_ARGS__);                           \
  ENSURE_CQMSAN_INITED();                                       \
  CQMSanInterceptorContext cqmsan_ctx = {IsInInterceptorScope()}; \
  ctx = (void *)&cqmsan_ctx;                                    \
  (void)ctx;                                                  \
  InterceptorScope interceptor_scope;                         \
  __cqmsan_unpoison(__errno_location(), sizeof(int));
...
#define COMMON_INTERCEPTOR_LIBRARY_LOADED(filename, handle)                    \
  do {                                                                         \
    ...
    if (filename && map)                                                       \
      ForEachMappedRegion(map, __cqmsan_unpoison);                               \
  } while (false)

#define COMMON_INTERCEPTOR_NOTHING_IS_INITIALIZED (!cqmsan_inited)
...
#define COMMON_INTERCEPTOR_MEMSET_IMPL(ctx, block, c, size) \
  {                                                         \
    (void)ctx;                                              \
    return __cqmsan_memset(block, c, size);                   \
  }
#define COMMON_INTERCEPTOR_MEMMOVE_IMPL(ctx, to, from, size) \
  {                                                          \
    (void)ctx;                                               \
    return __cqmsan_memmove(to, from, size);                   \
  }
#define COMMON_INTERCEPTOR_MEMCPY_IMPL(ctx, to, from, size) \
  {                                                         \
    (void)ctx;                                              \
    return __cqmsan_memcpy(to, from, size);                   \
  }
...
#define COMMON_INTERCEPTOR_COPY_STRING(ctx, to, from, size) \
  do {                                                      \
    ...
    /* To disable: (void)(ctx); (void)(to); (void)(from); (void)(size); */ \
  } while (false)
...
#define SIGNAL_INTERCEPTOR_ENTER() ENSURE_CQMSAN_INITED()
...
static int sigaction_impl(int signo, const __sanitizer_sigaction *act,
  ...
  if (flags()->wrap_signals) {
    ...
    __sanitizer::uptr old_cb = atomic_load(&sigactions[signo], memory_order_relaxed);
    __sanitizer_sigaction new_act;
    __sanitizer_sigaction *pnew_act = act ? &new_act : nullptr;
    if (act) {
      REAL(memcpy)(pnew_act, act, sizeof(__sanitizer_sigaction));
      __sanitizer::uptr cb = (__sanitizer::uptr)pnew_act->sigaction;
      __sanitizer::uptr new_cb = (pnew_act->sa_flags & __sanitizer::sa_siginfo)
                        ? (__sanitizer::uptr)SignalAction
                        : (__sanitizer::uptr)SignalHandler;
    ...
    if (res == 0 && oldact) {
      __sanitizer::uptr cb = (__sanitizer::uptr)oldact->sigaction;
      if (cb == (__sanitizer::uptr)SignalAction || cb == (__sanitizer::uptr)SignalHandler) {
  ...
  if (res == 0 && oldact) {
    __cqmsan_unpoison(oldact, sizeof(__sanitizer_sigaction));
  }
  return res;
}
...
static __sanitizer::uptr signal_impl(int signo, __sanitizer::uptr cb) {
  ...
  if (flags()->wrap_signals) {
    ...
    if (cb != __sanitizer::sig_ign && cb != __sanitizer::sig_dfl) {
      atomic_store(&sigactions[signo], cb, memory_order_relaxed);
      cb = (__sanitizer::uptr)&SignalHandler;
    }
  }
  return cb;
}
...
#define COMMON_SYSCALL_POST_WRITE_RANGE(p, s) __cqmsan_unpoison(p, s)
...
#  define CQMSAN_MAYBE_INTERCEPT_DLADDR1 CQMsan_INTERCEPT_FUNC(dladdr1)
#else
#define CQMSAN_MAYBE_INTERCEPT_DLADDR1
#endif
...
static int cqmsan_dl_iterate_phdr_cb(__sanitizer_dl_phdr_info *info, SIZE_T size,
                                   void *data) {
  if (info) {
    __cqmsan_unpoison(info, size);
    if (info->dlpi_phdr && info->dlpi_phnum)
      __cqmsan_unpoison(info->dlpi_phdr, struct_ElfW_Phdr_sz * info->dlpi_phnum);
    if (info->dlpi_name)
      __cqmsan_unpoison(info->dlpi_name, __sanitizer::internal_strlen(info->dlpi_name) + 1);
  }
...
INTERCEPTOR(void *, shmat, int shmid, const void *shmaddr, int shmflg) {
  ...
  if (p != (void *)-1) {
    ...
    if (!res) {
      __cqmsan_unpoison(p, ds.shm_segsz);
    }
  }
  return p;
}
...
INTERCEPTOR(int, dl_iterate_phdr, dl_iterate_phdr_cb callback, void *data) {
  ...
  cbdata.data = data;
  int res = REAL(dl_iterate_phdr)(cqmsan_dl_iterate_phdr_cb, (void *)&cbdata);
  return res;
}
...
INTERCEPTOR(wchar_t *, wcscpy, wchar_t *dest, const wchar_t *src) {
  ...
  //CopyShadowAndOrigin(dest, src, sizeof(wchar_t) * (internal_wcslen(src) + 1),
                      // &stack);
  __cqmsan_unpoison(dest, sizeof(wchar_t) * (internal_wcslen(src) + 1));
  return res;
}
...
INTERCEPTOR(wchar_t *, wcsncpy, wchar_t *dest, const wchar_t *src, SIZE_T n) {
  ...
  //__cqmsan_unpoison(dest, copy_size * sizeof(wchar_t));
  __cqmsan_unpoison(dest, n * sizeof(wchar_t));
  return res;
}
...
// [DONE]
void __cqmsan_unpoison_param(__sanitizer::uptr n) { UnpoisonParam(n); }
...
void *__cqmsan_memcpy(void *dest, const void *src, SIZE_T n) {
  if (!cqmsan_inited) return internal_memcpy(dest, src, n);
  if (cqmsan_init_is_running || __cqmsan::IsInSymbolizerOrUnwider())
  ...
  // New opportunistic code. We ignore the src's shadow. We directly assume that the copy makes 'dest' unpoisoned.
  __cqmsan_unpoison(dest, n);
  return res;
}

void *__cqmsan_memset(void *s, int c, SIZE_T n) {
  if (!cqmsan_inited) return internal_memset(s, c, n);
  if (cqmsan_init_is_running || __cqmsan::IsInSymbolizerOrUnwider())
    return REAL(memset)(s, c, n);
  ENSURE_CQMSAN_INITED();
  void *res = REAL(memset)(s, c, n);
  __cqmsan_unpoison(s, n);
  return res;
}

void *__cqmsan_memmove(void *dest, const void *src, SIZE_T n) {
  if (!cqmsan_inited) return internal_memmove(dest, src, n);
  if (cqmsan_init_is_running || __cqmsan::IsInSymbolizerOrUnwider())
  ...
  // Opportunistic
  // We ignore the src's shadow. We directly assume that the move makes 'dest' unpoisoned.
  __cqmsan_unpoison(dest, n);
  return res;
}
...
void InitializeInterceptors() {
  ...
  CQMSAN_MAYBE_INTERCEPT_MEMALIGN;
  CQMSAN_MAYBE_INTERCEPT___LIBC_MEMALIGN;
  INTERCEPT_FUNCTION(valloc);
  CQMSAN_MAYBE_INTERCEPT_PVALLOC;
  INTERCEPT_FUNCTION(malloc);
  INTERCEPT_FUNCTION(calloc);
  INTERCEPT_FUNCTION(realloc);
  INTERCEPT_FUNCTION(reallocarray);
  INTERCEPT_FUNCTION(free);
  CQMSAN_MAYBE_INTERCEPT_CFREE;
  CQMSAN_MAYBE_INTERCEPT_MALLOC_USABLE_SIZE;
  CQMSAN_MAYBE_INTERCEPT_MALLINFO;
  CQMSAN_MAYBE_INTERCEPT_MALLINFO2;
  CQMSAN_MAYBE_INTERCEPT_MALLOPT;
  CQMSAN_MAYBE_INTERCEPT_MALLOC_STATS;
  INTERCEPT_FUNCTION(fread);
  CQMSAN_MAYBE_INTERCEPT_FREAD_UNLOCKED;
  INTERCEPT_FUNCTION(memccpy);
  CQMSAN_MAYBE_INTERCEPT_MEMPCPY;
  INTERCEPT_FUNCTION(bcopy);
  INTERCEPT_FUNCTION(wmemset);
  INTERCEPT_FUNCTION(wmemcpy);
  CQMSAN_MAYBE_INTERCEPT_WMEMPCPY;
  INTERCEPT_FUNCTION(wmemmove);
  INTERCEPT_FUNCTION(strcpy);
  CQMSAN_MAYBE_INTERCEPT_STPCPY;
  CQMSAN_MAYBE_INTERCEPT_STPNCPY;
  INTERCEPT_FUNCTION(strdup);
  CQMSAN_MAYBE_INTERCEPT___STRDUP;
  INTERCEPT_FUNCTION(strncpy);
  CQMSAN_MAYBE_INTERCEPT_GCVT;
  ...
  CQMSAN_MAYBE_INTERCEPT___STRFTIME_L;
  INTERCEPT_FUNCTION(wcsftime);
  INTERCEPT_FUNCTION(wcsftime_l);
  CQMSAN_MAYBE_INTERCEPT___WCSFTIME_L;
  ...
  CQMSAN_MAYBE_INTERCEPT_FCVT;
  CQMSAN_MAYBE_INTERCEPT_FSTAT;
  CQMSAN_MAYBE_INTERCEPT_FSTAT64;
  CQMSAN_MAYBE_INTERCEPT___FXSTAT;
  CQMSAN_MAYBE_INTERCEPT_FSTATAT;
  CQMSAN_MAYBE_INTERCEPT_FSTATAT64;
  CQMSAN_MAYBE_INTERCEPT___FXSTATAT;
  CQMSAN_MAYBE_INTERCEPT___FXSTAT64;
  CQMSAN_MAYBE_INTERCEPT___FXSTATAT64;
  INTERCEPT_FUNCTION(pipe);
  INTERCEPT_FUNCTION(pipe2);
  INTERCEPT_FUNCTION(socketpair);
  CQMSAN_MAYBE_INTERCEPT_FGETS_UNLOCKED;
  INTERCEPT_FUNCTION(getrlimit);
  CQMSAN_MAYBE_INTERCEPT___GETRLIMIT;
  CQMSAN_MAYBE_INTERCEPT_GETRLIMIT64;
  CQMSAN_MAYBE_INTERCEPT_PRLIMIT;
  CQMSAN_MAYBE_INTERCEPT_PRLIMIT64;
  INTERCEPT_FUNCTION(gethostname);
  CQMSAN_MAYBE_INTERCEPT_EPOLL_WAIT;
  CQMSAN_MAYBE_INTERCEPT_EPOLL_PWAIT;
  INTERCEPT_FUNCTION(strsignal);
  INTERCEPT_FUNCTION(dladdr);
  CQMSAN_MAYBE_INTERCEPT_DLADDR1;
  ...
  CQMSAN_MAYBE_INTERCEPT_OPENPTY;
  CQMSAN_MAYBE_INTERCEPT_FORKPTY;

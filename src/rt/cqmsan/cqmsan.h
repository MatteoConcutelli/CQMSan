...
// cqmsan_allocator.cpp, which need to be kept in sync.
...
#define MEM_TO_SHADOW(mem) (((__sanitizer::uptr)(mem)) ^ 0x8000000000ULL)
#define SHADOW_TO_ORIGIN(shadow) (((__sanitizer::uptr)(shadow)) + 0x2000000000ULL)
...
# define MEM_TO_SHADOW(mem) ((__sanitizer::uptr)mem ^ 0xB00000000000ULL)
# define SHADOW_TO_ORIGIN(shadow) (((__sanitizer::uptr)(shadow)) + 0x200000000000ULL)
...
#  define MEM_TO_SHADOW(mem) (((__sanitizer::uptr)(mem)) ^ 0x500000000000ULL)
#  define SHADOW_TO_ORIGIN(shadow) (((__sanitizer::uptr)(shadow)) + 0x100000000000ULL)
...
#define LINEARIZE_MEM(mem) \
  (((__sanitizer::uptr)(mem) & ~0xE00000000000ULL) ^ 0x100000000000ULL)
#define MEM_TO_SHADOW(mem) (LINEARIZE_MEM((mem)) + 0x080000000000ULL)
#define SHADOW_TO_ORIGIN(shadow) (((__sanitizer::uptr)(shadow)) + 0x140000000000ULL)
...
#define MEM_TO_SHADOW(mem) \
  ((((__sanitizer::uptr)(mem)) & ~0xC00000000000ULL) + 0x080000000000ULL)
#define SHADOW_TO_ORIGIN(shadow) (((__sanitizer::uptr)(shadow)) + 0x140000000000ULL)
...
#define LINEARIZE_MEM(mem) \
  (((__sanitizer::uptr)(mem) & ~0x1800000000000ULL) ^ 0x400000000000ULL)
#define MEM_TO_SHADOW(mem) (LINEARIZE_MEM((mem)) + 0x200000000000ULL)
#define SHADOW_TO_ORIGIN(shadow) (((__sanitizer::uptr)(shadow)) + 0x500000000000)
...
#define LINEARIZE_MEM(mem) \
  (((__sanitizer::uptr)(mem) & ~0xc00000000000ULL) ^ 0x200000000000ULL)
#define MEM_TO_SHADOW(mem) (LINEARIZE_MEM((mem)) + 0x100000000000ULL)
#define SHADOW_TO_ORIGIN(shadow) (((__sanitizer::uptr)(shadow)) + 0x280000000000)
...
const MappingDesc kMemoryLayout[] = {
    ...
    {0x740000000000ULL, 0x800000000000ULL, MappingDesc::APP, "app-3"}};
#define MEM_TO_SHADOW(mem) (((__sanitizer::uptr)(mem)) ^ 0x500000000000ULL)
#define SHADOW_TO_ORIGIN(mem) (((__sanitizer::uptr)(mem)) + 0x100000000000ULL)

#else
#error "Unsupported platform"
#endif

const __sanitizer::uptr kMemoryLayoutSize = sizeof(kMemoryLayout) / sizeof(kMemoryLayout[0]);
...
#endif
inline bool
addr_is_type(__sanitizer::uptr addr, int mapping_types) {
...
#define MEM_IS_APP(mem) \
  (addr_is_type((__sanitizer::uptr)(mem), MappingDesc::APP | MappingDesc::ALLOCATOR))
#define MEM_IS_SHADOW(mem) addr_is_type((__sanitizer::uptr)(mem), MappingDesc::SHADOW)
#define MEM_IS_ORIGIN(mem) addr_is_type((__sanitizer::uptr)(mem), MappingDesc::ORIGIN)

// These constants must be kept in sync with the ones in MemorySanitizer.cpp.
const int kCQMsanParamTlsSize = 800;
const int kCQMsanRetvalTlsSize = 800;

namespace __cqmsan {
extern int cqmsan_inited;
extern bool cqmsan_init_is_running;
extern int cqmsan_report_count;

bool ProtectRange(__sanitizer::uptr beg, __sanitizer::uptr end);
bool InitShadowWithReExec(bool init_origins);
char *GetProcSelfMaps();
void InitializeInterceptors();

void CQMsanAllocatorInit();
void CQMsanDeallocate(__sanitizer::BufferedStackTrace *stack, void *ptr);

void *cqmsan_malloc(__sanitizer::uptr size, __sanitizer::BufferedStackTrace *stack);
void *cqmsan_calloc(__sanitizer::uptr nmemb, __sanitizer::uptr size, __sanitizer::BufferedStackTrace *stack);
void *cqmsan_realloc(void *ptr, __sanitizer::uptr size, __sanitizer::BufferedStackTrace *stack);
void *cqmsan_reallocarray(void *ptr, __sanitizer::uptr nmemb, __sanitizer::uptr size, __sanitizer::BufferedStackTrace *stack);
void *cqmsan_valloc(__sanitizer::uptr size, __sanitizer::BufferedStackTrace *stack);
void *cqmsan_pvalloc(__sanitizer::uptr size, __sanitizer::BufferedStackTrace *stack);
void *cqmsan_aligned_alloc(__sanitizer::uptr alignment, __sanitizer::uptr size, __sanitizer::BufferedStackTrace *stack);
...
void PrintWarning(__sanitizer::uptr pc, __sanitizer::uptr bp);
...
const int STACK_TRACE_TAG_POISON = __sanitizer::StackTrace::TAG_CUSTOM + 1;
...
#define GET_MALLOC_STACK_TRACE                                             \
  ...
  if (__cqmsan_get_track_origins() && cqmsan_inited) {                         \
    stack.Unwind(__sanitizer::StackTrace::GetCurrentPc(), GET_CURRENT_FRAME(), nullptr, \
...
#define GET_STORE_STACK_TRACE_PC_BP(pc, bp)                              \
  UNINITIALIZED __sanitizer::BufferedStackTrace stack;                                \
  if (__cqmsan_get_track_origins() > 1 && cqmsan_inited) {                   \
    int size = flags()->store_context_size;                              \
    if (!SANITIZER_CAN_FAST_UNWIND)                                      \
      size = Min(size, 1);                                               \
    stack.Unwind(pc, bp, nullptr, __sanitizer::common_flags()->fast_unwind_on_malloc, \
                 size);                                                  \
  }

#define GET_STORE_STACK_TRACE \
  GET_STORE_STACK_TRACE_PC_BP(__sanitizer::StackTrace::GetCurrentPc(), GET_CURRENT_FRAME())

#define GET_FATAL_STACK_TRACE_PC_BP(pc, bp)                              \
  UNINITIALIZED __sanitizer::BufferedStackTrace stack;                                \
  if (cqmsan_inited) {                                                     \
    stack.Unwind(pc, bp, nullptr, __sanitizer::common_flags()->fast_unwind_on_fatal); \
  }
...
void *CQMsanTSDGet();
void CQMsanTSDSet(void *tsd);
void CQMsanTSDDtor(void *tsd);

void InstallAtForkHandler();

// Dedup UMR per pc (definita in cqmsan.cpp). Vedi commenti nel .cpp.
//bool IsUMRSeen(__sanitizer::uptr pc);
//bool MarkUMRSeen(__sanitizer::uptr pc);

}  // namespace __cqmsan

// --- API PUBBLICA (Esportata per il Runner/User) ---
// Queste devono stare fuori dal namespace o essere chiaramente globali
extern "C" {
    // Prototipi (senza corpo!)

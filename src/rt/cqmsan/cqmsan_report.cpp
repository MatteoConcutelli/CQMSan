...
static void DescribeStackOrigin(const char *so, __sanitizer::uptr pc) {
  Decorator d;
  Printf("%s", d.Origin());
  if (so) {
    Printf(
...
static void DescribeOrigin(__sanitizer::u32 id) {
  ...
  if (o.isStackOrigin()) {
    ...
    __sanitizer::StackTrace stack = o.getStackTraceForHeapOrigin();
    switch (stack.tag) {
...
void ReportUMR(__sanitizer::StackTrace*stack, __sanitizer::u32 origin) {
  ...
  Decorator d;
  Printf("%s", d.Warning());
  Report("WARNING: CompilerQEMUMemorySanitizer: use-of-uninitialized-value\n");
  Printf("%s", d.Default());
  stack->Print();
  if (origin) {
...
void ReportStats() {
  ...
  if (__cqmsan_get_track_origins() > 0) {
    __sanitizer::StackDepotStats stack_depot_stats = StackDepotGetStats();
    ...
    __sanitizer::StackDepotStats chained_origin_depot_stats = ChainedOriginDepotGetStats();
...
void ReportAtExitStatistics() {
  ...
  if (cqmsan_report_count > 0) {
    Decorator d;
    Printf("%s", d.Warning());
    Printf("CompilerQEMUMemorySanitizer: %d warnings reported.\n", cqmsan_report_count);
    Printf("%s", d.Default());
  }
}
...
class OriginSet {
 public:
  ...
  __sanitizer::u32 get(int id) { return origins_[id]; }
 ...
 private:
  ...
  __sanitizer::u32 origins_[kMaxSize_];
  int next_id_;
};

void DescribeMemoryRange(const void *x, __sanitizer::uptr size) {
  // Real limits.
  __sanitizer::uptr start = MEM_TO_SHADOW(x);
  __sanitizer::uptr end = start + size;
  // Scan limits: align start down to 4; align size up to 16.
  __sanitizer::uptr s = start & ~3UL;
  size = end - s;
  size = (size + 15) & ~15UL;
  __sanitizer::uptr e = s + size;

  // Single letter names to origin id mapping.
  OriginSet origin_set;

  __sanitizer::uptr pos = 0;  // Offset from aligned start.
  bool with_origins = __cqmsan_get_track_origins();
  ...
  Decorator d;
  Printf("%s", d.Warning());
  __sanitizer::uptr start_x = reinterpret_cast<__sanitizer::uptr>(x);
  ...
  while (s < e) {
    ...
    if (pos % 4 == 3 && with_origins) {
      ...
      if (last_quad_poisoned) {
        __sanitizer::u32 o = *(__sanitizer::u32 *)SHADOW_TO_ORIGIN(s - 3);
  ...
  for (int i = 0; i < origin_set.size(); ++i) {
    __sanitizer::u32 o = origin_set.get(i);
